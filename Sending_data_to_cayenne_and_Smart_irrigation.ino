...
Windows:
  ...
  2. Navigate to the scripts folder by typing "cd [path]", e.g.  "cd C:\Users\lbahja-pc\Documents\Arduino\libraries\CayenneMQTT\extras\scripts"
...
// you can find username and password and cliendID when you create an account on cayenne plateforme
...
CAYENNE_OUT_DEFAULT()
{
  ...
  Cayenne.virtualWrite(0, soilHum, "soil_moist", "p");
  delay(5000);
...
void readSensors(void)
{
  ...
  soilHum = map(analogRead(A0), 1024,0, 0, 100);
...
void displaySensors(void)
{
  ...
  if( soilHum<map(614.4 , 1024, 0, 0, 100)){
     analogWrite(ENA, 255);
  //analogWrite(ENB, 255); 
